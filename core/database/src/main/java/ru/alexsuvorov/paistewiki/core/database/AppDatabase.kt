package ru.alexsuvorov.paistewiki.core.database
...
import ru.alexsuvorov.paistewiki.core.database.dao.MonthDao
import ru.alexsuvorov.paistewiki.core.database.dao.NewsDao
import ru.alexsuvorov.paistewiki.core.database.dao.SupportAnatomyDao
import ru.alexsuvorov.paistewiki.core.database.dao.SupportDao
import ru.alexsuvorov.paistewiki.core.database.framework.AssetSQLiteOpenHelperFactory
...
@Database(
    entities = [CymbalSeries::class, News::class, Month::class, SupportModel::class, SupportAnatomy::class],
    version = 11,
    exportSchema = false
)
    ...
    companion object {
        ...
        fun getDatabase(context: Context): AppDatabase {
            return INSTANCE ?: synchronized(this) {
                val instance = openDatabase(context)
                INSTANCE = instance
                instance
            }
        }
        ...
        private fun openDatabase(context: Context): AppDatabase {
            // Get language from AppCompatDelegate instead of AppPreferences
            val currentLocales = AppCompatDelegate.getApplicationLocales()
            var choosedLang = if (currentLocales.isEmpty) "en" else currentLocales.get(0)?.language ?: "en"

            println("TEST111 DATABASE LANGUAGE = '$choosedLang'")

            DATABASE_LANGUAGE = choosedLang
            val databaseName = "cymbalsbase_${choosedLang.uppercase()}.db"
        ...
        private val MIGRATION_2_3 = object : Migration(2, 3) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("CREATE TABLE IF NOT EXISTS 'cymbalseries' ( 'cymbalseries_id' INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, 'cymbalseries_name' TEXT NOT NULL, 'cymbalseries_subname' TEXT, 'cymbalseries_singleimageuri' TEXT, 'cymbalseries_imageuri' TEXT )")
            }
        }
        ...
        private val MIGRATION_5_6 = object : Migration(5, 6) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("CREATE TABLE IF NOT EXISTS 'news_table' ( 'news_id' INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, 'news_title' TEXT, 'news_category' TEXT, 'news_url' TEXT, 'news_index' INTEGER NOT NULL)")
                database.execSQL("CREATE TABLE IF NOT EXISTS 'news_month_table' ( 'month_id' INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, 'month_name' TEXT, 'month_url' TEXT, 'month_index' INTEGER NOT NULL)")
            }
        }

        private val MIGRATION_6_7 = object : Migration(6, 7) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS 'index_news_table_news_index' ON 'news_table' ( 'news_url' )")
                database.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS 'index_news_month_table_month_index' ON 'news_month_table' ( 'month_index' )")
            }
        }
        ...
        private val MIGRATION_9_10 = object : Migration(9, 10) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("CREATE TABLE IF NOT EXISTS 'support_table' ('support_id' INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, 'support_title' TEXT, 'support_text' TEXT, 'support_image' TEXT);")
                database.execSQL("CREATE TABLE IF NOT EXISTS 'support_anatomy' ('anatomy_id' INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, 'anatomy_title' TEXT, 'anatomy_subtitle' TEXT, 'anatomy_text' TEXT);")
            }
        }
        ...
        private val MIGRATION_10_11 = object : Migration(10, 11) {
            override fun migrate(database: SupportSQLiteDatabase) {
                if (DATABASE_LANGUAGE == "ru") {
                    ...
                    database.execSQL("INSERT OR REPLACE INTO 'support_anatomy' VALUES (9,'Hi-Hat','Hi-Hats are two paired cymbals.','Which are mounted opposite each other on a hi-hat stand. They can be closed or clashed together with a foot pedal. The top cymbal is usually thinner to make the hi-hat more responsive to stick work, and the two cymbals are usually quite different in sound. Hi-Hats can be played in a variety of ways: with the stick on the closed surface for distinct rhythmic patterns, half open on the surface or across the edge for full sounding rhythmic patterns. Closing the hi-hat rapidly produces the very short chick sound, which is used for rhythmic patterns, usually in context with ride, snare, and base drum. They can also be clashed together to produce a crash sound.');")
