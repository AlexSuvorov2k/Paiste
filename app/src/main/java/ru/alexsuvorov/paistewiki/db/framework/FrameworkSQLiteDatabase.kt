 ...
 */(private val mDelegate: SQLiteDatabase) : SupportSQLiteDatabase {
    ...
    override fun query(supportQuery: SupportSQLiteQuery): Cursor {
        return mDelegate.rawQueryWithFactory(SQLiteDatabase.CursorFactory { db: SQLiteDatabase?, masterQuery: SQLiteCursorDriver?, editTable: String?, query: SQLiteQuery? ->
            supportQuery.bindTo(FrameworkSQLiteProgram(query!!))
            SQLiteCursor(masterQuery, editTable, query)
        }, supportQuery.sql, EMPTY_STRING_ARRAY, "")
    }
    ...
    override fun query(
        ...
        return mDelegate.rawQueryWithFactory(
            SQLiteDatabase.CursorFactory { db: SQLiteDatabase?,
                                           masterQuery: SQLiteCursorDriver?,
                                           editTable: String?,
                                           query: SQLiteQuery? ->
                supportQuery.bindTo(FrameworkSQLiteProgram(query!!))
                SQLiteCursor(masterQuery, editTable, query)
            }, supportQuery.sql, EMPTY_STRING_ARRAY, "", cancellationSignal
        )
    }
    ...
    companion object {
        private val CONFLICT_VALUES: Array<String> = arrayOf<String>("", " OR ROLLBACK ", " OR ABORT ", " OR FAIL ", " OR IGNORE ", " OR REPLACE ")
